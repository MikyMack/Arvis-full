const MainBanner = require('../models/BannerFour');
...
function extractCloudinaryPublicId(imageUrl, folder = 'main-banners') {
  ...
  try {
    ...
    const hofmaanIdx = parts.findIndex(p => p === 'Arvies');
    if (hofmaanIdx === -1) return null;
    const folderParts = [ 'Arvies', folder ];
